  ...
  return (
    <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
      <div style={{ marginBottom: "2.5rem" }}>
        ...
        <p style={{ fontSize: "1.2rem", color: "var(--text-muted)", margin: 0 }}>AIRANK 서비스의 전체 운영 데이터를 한눈에 확인합니다.</p>
      </div>
        ...
        {STATS.map(({ label, col, icon, color }) => (
          <div key={col} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "1.5rem" }}>
              <span style={{ fontSize: "1.6rem", background: `${color}15`, padding: "8px", borderRadius: "0" }}>{icon}</span>
              <span style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" }}>{label}</span>
            </div>
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: "1.8rem", fontWeight: 800, color: "var(--text-primary)", marginBottom: "1.5rem" }}>운영 도구 바로가기</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "12px" }}>
          {[
            ...
            >
              <div style={{ fontWeight: 800, color: "var(--text-primary)", fontSize: "1.3rem" }}>{label}</div>
