...
const EditorArea = styled.div`
  ...
  img { max-width: 100%; border-radius: var(--r-sm); display: block; margin: 1rem 0; cursor: move; transition: transform 0.2s; }
  img:hover { transform: scale(1.002); outline: 2px solid var(--accent-indigo); }
...
const ResizeHandle = styled.div`
  ...
  background: var(--accent-indigo); border: 2px solid #fff; border-radius: 2px;
  cursor: nwse-resize; z-index: 1001; pointer-events: auto;
`;
...
export default function RichEditor({ value, onChange, placeholder = "내용을 입력하세요...", postKey }) {
  ...
  const [imgBubble, setImgBubble] = useState(null); // { x, y, target, width, height }
    ...
    <Wrap onClick={(e) => { if (activePanel && !e.target.closest('.pro-panel')) setActivePanel(null); }}>
      ...
      <div style={{ position: "relative" }}>
        <EditorArea
          ...
          onMouseUp={(e) => {
            handleSelectionChange();
            if (resizing) { setResizing(null); updateCount(); }
          }}
          onMouseMove={(e) => {
            if (resizing) {
              const deltaX = e.clientX - resizing.startX;
              const newWidth = Math.max(50, resizing.startWidth + deltaX);
              resizing.target.style.width = `${newWidth}px`;
              // 버블 위치 업데이트를 위해 강제로 리렌더링 유도
              setImgBubble(prev => ({ ...prev }));
            }
          }}
          onDragOver={(e) => {
            // 외부 파일이 드래그 중인 경우에만 기본 동작 방지 (드롭 활성화)
        ...
        {imgBubble && (
          <ResizeHandle 
            style={{ 
              left: imgBubble.target.offsetLeft + imgBubble.target.offsetWidth - 7, 
              top: imgBubble.target.offsetTop + imgBubble.target.offsetHeight - 7 
            }} 
            onMouseDown={(e) => {
              e.preventDefault(); e.stopPropagation();
              setResizing({ startX: e.clientX, startWidth: imgBubble.target.offsetWidth, target: imgBubble.target });
            }}
          />
        )}
      </div>
      ...
      <div style={{ marginTop: "1rem" }}>
        <FileUploader
          ...
          onInsertImage={(url, name) => {
            editorRef.current?.focus();
            document.execCommand("insertHTML", false, `<img src="${url}" alt="${name}" title="클릭하여 크기/배치 조절" draggable="true" /><p><br></p>`);
            updateCount();
          }}
        />
      </div>
